...
class BoosterT1JumpEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        # ======================================================================================
        # PhysX Settings - Increase buffers for rough terrain with many environments
        # ======================================================================================
        # Rough terrain with many parallel environments requires larger PhysX buffers
...
@configclass
class BoosterT1JumpEnvCfg_H100(BoosterT1JumpEnvCfg):
    """Optimized environment configuration for NVIDIA H100 80GB HBM3.

    This configuration maximizes GPU utilization for the H100 by:
    - Increasing parallel environments from 4096 to 16384 (4x)
    - Scaling PhysX buffers proportionally
    - Optimizing memory usage for 80GB VRAM
    """
    ...
    def __post_init__(self):
        ...
        # ======================================================================================
        # H100-Optimized Scene Settings
        # ======================================================================================
        # H100 can handle 4x more environments than default
        self.scene.num_envs = 16384  # Increased from 4096
        self.scene.env_spacing = 2.5  # Keep spacing same

        # ======================================================================================
        # H100-Optimized PhysX Settings
        # ======================================================================================
        # Scale buffers proportionally for 4x environments

        # Patch buffer: scale to 44 * 2^15 = 1,441,792 (4x base for 16K envs)
        self.sim.physx.gpu_max_rigid_patch_count = 44 * 2**15

        # Collision stack: scale to 256 MB for 16K environments
        self.sim.physx.gpu_collision_stack_size = 256 * 2**20

        # Increase contact buffer capacity for more environments
        self.sim.physx.gpu_max_rigid_contact_count = 2**24  # Increased from 2^23

        # Increase heap capacity for better memory management
        self.sim.physx.gpu_heap_capacity = 2**27  # Increased from 2^26

        # Temp buffer for intermediate calculations
        self.sim.physx.gpu_temp_buffer_capacity = 2**25  # Increased from 2^24
